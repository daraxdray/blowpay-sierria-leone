...
import { BLACK, PRIMARY_COLOR, SECONDARY_COLOR } from '../../global/theme';
const DateModal = ({ visible, onClose, onDateChange, selectedDate }) => {
  ...
  const renderHeader = () => {
    ...
    const { year, month, day } = parseDate(tempDate || new Date().toISOString().split('T')[0].replace(/-/g, '/'));

    return (
      <View style={tw`bg-[${SECONDARY_COLOR}] p-4 rounded-t-[10px]`}>
        <Text style={tw`text-white text-1xl text[#333333] font-bold`}>Select Date</Text>
        ...
        <View style={tw`bg-white rounded-t-[20px] overflow-hidden`}>
          ...
          <DatePicker
            ...
            options={{
              ...
              textHeaderColor: '#333333',      // Dark text for headers
              textDefaultColor: '#212529',     // Very dark gray for default text
              selectedTextColor: '#FFFFFF',    // White for selected text
              mainColor: '#FF114A',            // Your primary red for highlights
              textSecondaryColor: '#6C757D',   // Muted gray for secondary elements
              selectedBackgroundColor: '#FF114A'
            }}
            style={tw`p-4`}
          />

          <View style={tw`flex-row justify-end gap-4 p-4 border-t border-gray-200`}>
            <TouchableOpacity
