...
import { useRegisterUser, useResetPassword } from '../../../hooks/auth.hook';
...
const ResetPassword = ({ navigation, route }) => {
  ...
  const { mutate, status, isLoading } = useResetPassword();
  ...
  const handleRegister = values => {
    ...
    mutate(userData, {
      onSuccess: data => {
        console.log(data);
        if (data) {
          navigation.navigate('signin-screen', { emailAddress });
        } else {
          Toast.show({
            type: 'error',
            text1: 'Password Reset Failed',
            text2:
              'Something went wrong in reseting your password, please try again.',
          });
        }
      },
      onError: error => {
